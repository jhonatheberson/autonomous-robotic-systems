...
%%% 1st Config %%%%
rectangle('Position',[-1 -0.5 1  1], 'FaceColor',[0 .5 .5])
axis([-2.5 2.5 -2.5 2.5])
axis square
% 
hold on
rectangle('Position',[1 0.5 1 0.5], 'FaceColor',[0 .5 .5])
%%%%%%%%%%%%%%%%%%%
...
circles(start(1), start(2),0.2, 'facecolor','green')

% Plotting goal position
circles(goal(1), goal(2),0.2, 'facecolor','yellow')

%% initialising the hash map

%%% 1st Config
keys = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'};
values = {start, [-1,-0.5], [-1,0.5], [0,-0.5], [0,0.5], [1,0.5], [1,1], [2,0.5], [2,1], goal};
%%%%%%%%%%%%%%%%%
...
for i=1:pathSize(2)-1
    ...
    title('Grafos de Visibilidade')
end
X(pathSize(2)) = p2(1);
Y(pathSize(2)) = p2(2);
Z(pathSize(2)) = 0.0;
MAT = [X' Y' Z'];
writematrix(MAT,'Curva Grafos de Visibilidade.csv')
toc
totalDis
%%%%%%%%%%%%%% end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
